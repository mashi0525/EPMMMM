...
Public Class HelperDatabase
    ...
    Public Shared Sub SaveBookingServices(bookingId As Integer, chkCatering As CheckBox, chkClown As CheckBox, chkSinger As CheckBox, chkDancer As CheckBox, chkVideoke As CheckBox)
            ...
            Try
                ...
                Using cmd As New MySqlCommand(insertQuery, connection)
                    ...
                    If chkDancer.Checked Then
                        cmd.Parameters.AddWithValue("@serviceId", 4)
                        cmd.ExecuteNonQuery()
                        cmd.Parameters.RemoveAt("@serviceId")
                    End If
                    If chkVideoke.Checked Then
                        cmd.Parameters.AddWithValue("@serviceId", 5)
                        cmd.ExecuteNonQuery()
                        cmd.Parameters.RemoveAt("@serviceId")
                    End If
                End Using
    ...
    Public Shared Function LoadBookedDates(placeId As Integer) As List(Of Date)
        ...
        For Each row As DataRow In dt.Rows
            bookedDates.Add(Convert.ToDateTime(row("event_date")))
        Next
